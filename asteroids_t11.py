...
class World(object):
    ...
    def handle_keyup(self, event):
        ...
        if event.key == pygame.K_SPACE:
            bullet = Bullet((Vector(*self.player.rect.center)-self.player.facing*2).to_position(),self.player.facing.to_degrees()[0]+180,self.player.motion.magnitude()+10)
...
def collision_player():
    for y in ([x for x in world.sprites if isinstance(x, Asteroid)]):
        if abs(y.rect.center[0]-world.player.rect.center[0])<15:
            if abs(y.rect.center[1]-world.player.rect.center[1])<18:
...
def collision_bullet():
    for i in ([x for x in world.sprites if isinstance(x, Asteroid)]):
        for y in ([x for x in world.sprites if isinstance(x, Bullet)]):
            if abs(i.rect.center[0]-y.rect.center[0])<15:
                if abs(i.rect.center[1]-y.rect.center[1])<15:
                    i.duration=0
                    y.duration=0
                    print "+1 punto"
...
def main():
    ...
    updateCollision = Thread(target=update_collisions)
    supdate.start()
